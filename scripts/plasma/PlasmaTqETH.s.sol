...
contract PlasmaTqETHDeploy is Script, Test {
    // Actors
    ...
    uint256 public constant INITIAL_WETH_PRICE = 991185691894931485;
    uint256 public constant INITIAL_USDT_PRICE = 465711030675126000000000000;
    ...
    function run() external {
        ...
        // Oracle assets: WETH and USDT
        address[] memory oracleAssets = ArraysLibrary.makeAddressArray(abi.encode(Constants.WETH, Constants.USDT0));
        ...
        // Risk manager allowed assets: WETH and USDT
        address[] memory riskManagerAssets = ArraysLibrary.makeAddressArray(abi.encode(Constants.WETH, Constants.USDT0));
        ...
        // Oracle: submit initial price reports (accept post-deploy via activeAdmin)
        {
            IOracle oracle = vault.oracle();

            IOracle.Report[] memory reports = new IOracle.Report[](2);
            reports[0].asset = Constants.WETH;
            reports[0].priceD18 = uint224(INITIAL_WETH_PRICE);
            reports[1].asset = Constants.USDT0;
            reports[1].priceD18 = uint224(INITIAL_USDT_PRICE);
    ...
    function _deploySwapModule(ProtocolDeployment memory $, address subvault) internal returns (address) {
        // Tokens: USDe, USDT, WETH, WXPL, XPL, sUSDe as tokenIn and tokenOut
        // Routers: KyberSwap + WXPL
        address[] memory actors = ArraysLibrary.makeAddressArray(
            abi.encode(
                // CALLER
                curator,
                // TOKEN_IN: USDe, USDT, WETH, WXPL, XPL, sUSDe
                Constants.USDE,
                Constants.USDT0,
                Constants.WETH,
                Constants.WXPL,
                Constants.XPL,
                Constants.SUSDE,
                // TOKEN_OUT: USDe, USDT, WETH, WXPL, XPL, sUSDe
                Constants.USDE,
                Constants.USDT0,
                Constants.WETH,
                Constants.WXPL,
