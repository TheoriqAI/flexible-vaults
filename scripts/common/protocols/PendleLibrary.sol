...
library PendleLibrary {
    ...
    struct PTStrategy {
        ...
        address[] inputTokens;    // Array of allowed input tokens (e.g., USDe, sUSDe)
        address mintSyToken;      // Token used for minting SY (e.g., sUSDe)
    }
        ...
        returns (IVerifier.VerificationPayload[] memory leaves)
    {
        ...
        // - 1 exitPostExpToToken
        uint256 length = totalInputTokens * 2 + $.strategies.length * 3;
        ...
        for (uint256 i = 0; i < $.strategies.length; i++) {
            ...
            // 4. Generate swapExactPtForToken proof (sells PT back to token)
            leaves[index++] = _makeSwapPtForTokenProof(
                ...
                $.subvault,
                strategy.market,
                strategy.mintSyToken
            );

            // 5. Generate exitPostExpToToken proof (redeems expired PT)
        ...
        returns (string[] memory descriptions)
    {
        ...
        // Count: (approvals + swaps for each input) + (PT approval + PT swap + PT exit)
        uint256 length = totalInputTokens * 2 + $.strategies.length * 3;
        ...
        for (uint256 i = 0; i < $.strategies.length; i++) {
            ...
            // PT token approval
            {
                string memory tokenSymbol = _tokenSymbol(strategy.mintSyToken);
    ...
    function _makeSwapPtForTokenProof(
        ...
        IPendleRouter.TokenOutput memory tokenOutput = IPendleRouter.TokenOutput({
            tokenOut: mintSyToken,       // Output token (template)
            minTokenOut: 0,              // ALLOW ANY
            tokenRedeemSy: mintSyToken,  // LOCKED: Must be mintSyToken
            pendleSwap: address(0),      // LOCKED: No Pendle swap
            swapData: swapData           // LOCKED: No external swaps
        });
        ...
        // Create mask (same strategy as swapTokenForPt)
        IPendleRouter.SwapData memory swapDataMask = IPendleRouter.SwapData({
        ...
        IPendleRouter.TokenOutput memory tokenOutputMask = IPendleRouter.TokenOutput({
            tokenOut: address(0),                   // ALLOW: Any output token
            minTokenOut: 0,                         // ALLOW: Any amount
            tokenRedeemSy: tokenOutput.tokenRedeemSy,  // LOCK: Must be mintSyToken
            pendleSwap: tokenOutput.pendleSwap,     // LOCK: Must be address(0)
            swapData: swapDataMask
        });
    ...
    function _makeExitPostExpProof(
        ...
        IPendleRouter.TokenOutput memory tokenOutput = IPendleRouter.TokenOutput({
            tokenOut: mintSyToken,
            minTokenOut: 0,
            tokenRedeemSy: mintSyToken,
        ...
        IPendleRouter.TokenOutput memory tokenOutputMask = IPendleRouter.TokenOutput({
            ...
            pendleSwap: tokenOutput.pendleSwap,
            swapData: swapDataMask
        });
