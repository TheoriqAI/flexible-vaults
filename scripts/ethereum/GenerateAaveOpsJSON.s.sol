...
contract GenerateAaveOpsJSON is Script, Test {
    ...
    /// @notice Helper: Generate Aave Core ops for PROD subvault 3
    /// @param categoryId eMode category ID for Aave
    function generateProdSv3Aave(uint8 categoryId) public {
        // Collaterals: WETH, wstETH (same as preprod)
        address[] memory collaterals = new address[](2);
        collaterals[0] = Constants.WETH;
        collaterals[1] = Constants.WSTETH;
    ...
    /// @notice Helper: Generate Spark ops for PROD subvault 3
    /// @param categoryId eMode category ID for Spark
    function generateProdSv3Spark(uint8 categoryId) public {
        // Collaterals: WETH, wstETH (same as preprod)
        address[] memory collaterals = new address[](2);
        collaterals[0] = Constants.WETH;
        collaterals[1] = Constants.WSTETH;

        // Borrows: WETH, wstETH, USDC, USDT, USDE (same as preprod)
        address[] memory borrows = new address[](5);
        borrows[0] = Constants.WETH;
        borrows[1] = Constants.WSTETH;
        borrows[2] = Constants.USDC;
    ...
    /// @notice Generate ALL prod sv3 ops with eMode 1 (Aave + Spark)
    function generateProdSv3AllEMode1() public {
        generateProdSv3Aave(1);
        generateProdSv3Spark(1);
    }

    /// @notice Helper: Generate Aave ops for PROD subvault 4 with PT tokens
    /// @param categoryId eMode category ID
    function generateProdSv4Aave(uint8 categoryId) public {
        // Collaterals: wstETH, PT-sUSDe
        address[] memory collaterals = new address[](2);
        collaterals[0] = Constants.WSTETH; // 0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0
        collaterals[1] = 0xE8483517077afa11A9B07f849cee2552f040d7b2; // PT-sUSDe-05FEB2026 (checksummed)

        // Borrows: USDe only
        address[] memory borrows = new address[](1);
        borrows[0] = Constants.USDE; // 0x4c9EDD5852cd905f086C759E8383e09bff1E68B3
    ...
    /// @notice Helper: Generate Aave ops for PreProd subvault 2 with sUSDe+wstETH collateral, USDe borrow
    /// @param categoryId eMode category ID (use 2 for sUSDe-related eMode)
    function generatePreProdSv2Aave(uint8 categoryId) public {
        // Collaterals: sUSDe, wstETH
        address[] memory collaterals = new address[](2);
        collaterals[0] = Constants.SUSDE;  // 0x9D39A5DE30e57443BfF2A8307A4256c8797A3497
        collaterals[1] = Constants.WSTETH; // 0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0
    ...
    /// @notice Helper: Generate Aave ops for Prod subvault 4 with sUSDe+wstETH collateral, USDe+USDC+USDT borrow
    /// @param categoryId eMode category ID (use 17 for USDe-specific eMode)
    function generateProdSv4AaveEMode2(uint8 categoryId) public {
        // Collaterals: sUSDe, wstETH
        address[] memory collaterals = new address[](2);
        collaterals[0] = Constants.SUSDE;  // 0x9D39A5DE30e57443BfF2A8307A4256c8797A3497
        collaterals[1] = Constants.WSTETH; // 0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0

        // Borrows: USDe, USDC, and USDT (all eligible in eMode 17)
        address[] memory borrows = new address[](3);
        borrows[0] = Constants.USDE; // 0x4c9EDD5852cd905f086C759E8383e09bff1E68B3
        borrows[1] = Constants.USDC; // 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48
        borrows[2] = Constants.USDT; // 0xdAC17F958D2ee523a2206206994597C13D831ec7
    ...
    /// @notice Helper: Generate Spark ops for PROD subvault 4 (eMode 0, no eMode)
    /// @dev Collaterals: wstETH, WETH, USDC, USDT. Loans: USDC, USDT.
    /// @param categoryId eMode category ID (pass 0 for no eMode)
    function generateProdSv4SparkEMode0(uint8 categoryId) public {
        // Collaterals: wstETH, WETH, USDC, USDT
        address[] memory collaterals = new address[](4);
        collaterals[0] = Constants.WSTETH;
        collaterals[1] = Constants.WETH;
        collaterals[2] = Constants.USDC;
        collaterals[3] = Constants.USDT;

        // Borrows: USDC, USDT
        address[] memory borrows = new address[](2);
        borrows[0] = Constants.USDC;
        borrows[1] = Constants.USDT;
    ...
    /// @notice Helper: Generate Aave ops for Prod subvault 4 with sUSDe + PT-srUSDe + wstETH collateral (eMode 44)
    /// @param categoryId eMode category ID (use 44)
    function generateProdSv4AaveEMode44(uint8 categoryId) public {
        // Collaterals: sUSDe, PT-srUSDe-24JUN2026, wstETH, WETH
        address[] memory collaterals = new address[](4);
        collaterals[0] = Constants.SUSDE;              // 0x9D39A5DE30e57443BfF2A8307A4256c8797A3497
        collaterals[1] = Constants.PT_SRUSDE_24JUN2026; // 0x619D75E3b790eBC21c289f2805Bb7177A7D732E2
        collaterals[2] = Constants.WSTETH;             // 0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0
        collaterals[3] = Constants.WETH;               // 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    ...
    function generateProdSv2Aave(uint8 categoryId) public {
        ...
        // Collaterals: WETH, wstETH (same as sv3)
        address[] memory collaterals = new address[](2);
        collaterals[0] = Constants.WETH;
        collaterals[1] = Constants.WSTETH;

        // Borrows: WETH, wstETH, USDC, USDT, USDE, EURC (same as sv3)
        address[] memory borrows = new address[](6);
        borrows[0] = Constants.WETH;
        borrows[1] = Constants.WSTETH;
        borrows[2] = Constants.USDC;
        borrows[3] = Constants.USDT;
        borrows[4] = Constants.USDE;
        borrows[5] = Constants.EURC;

        string memory title = string(
            abi.encodePacked("prod/tqETH/sv2-aaveOps-emode", vm.toString(uint256(categoryId)))
        );
        string memory subvaultName = "subvault2_aave";

        console.log("=== Generating Aave Operations JSON (Prod SV2) ===");
    ...
    /// @notice Generate Aave JSON with custom assets for MULTIPLE callers (entries doubled)
    /// @param subvaultIndex The subvault index
    /// @param isProd true for prod vault, false for preprod vault
    /// @param pool Aave pool address
    /// @param collaterals Array of collateral asset addresses
    /// @param borrows Array of borrow asset addresses
    /// @param outputSuffix Suffix for output filename
    /// @param categoryId Aave eMode category ID
    /// @param callers Array of caller addresses (e.g., [curator, executor])
    function generateWithCustomAssetsMultiCaller(
        uint256 subvaultIndex,
        bool isProd,
        address pool,
        address[] memory collaterals,
        ...
        console.log("=== Generating Aave Operations JSON (Multi-Caller) ===");
        console.log("Environment:", env);
        console.log("Subvault index:", subvaultIndex);
        console.log("Subvault address:", subvault);
        console.log("Pool:", pool);
        ...
        // Generate proofs for ALL callers (doubled entries)
    ...
    /// @notice Generate Aave ops for preprod sv4 with BOTH curator and executor as callers
    /// @param categoryId eMode category ID
    function generatePreProdSv4AaveMultiCaller(uint8 categoryId) public {
        address[] memory collaterals = new address[](3);
        collaterals[0] = Constants.WSTETH;
        collaterals[1] = 0x1F84a51296691320478c98b8d77f2Bbd17D34350; // PT-USDE
        collaterals[2] = 0xE8483517077afa11A9B07f849cee2552f040d7b2; // PT-sUSDe

        address[] memory borrows = new address[](3);
        borrows[0] = Constants.USDC;
        borrows[1] = Constants.USDT;
        borrows[2] = Constants.USDE;
        ...
        string memory suffix = string(abi.encodePacked("aaveOps-emode", vm.toString(uint256(categoryId))));
        generateWithCustomAssetsMultiCaller(
            4, // subvault 4
            false, // preprod
            Constants.AAVE_CORE,
            collaterals,
