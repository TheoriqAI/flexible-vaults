...
def merge_jsons(output_title: str, input_files: List[str]) -> dict:
    ...
    # Build source file provenance per input file so the output records exactly
    # which files + how many ops each contributed. Kept in a separate top-level
    # key so tests / on-chain consumers that only read merkle_root and
    # merkle_proofs are unaffected.
    sources = []
    for filepath in input_files:
        src = load_json(filepath)
        src_proofs = src.get('merkle_proofs', [])
        sources.append({
            'path': filepath,
            'filename': os.path.basename(filepath),
            'title': src.get('title'),
            'merkle_root': src.get('merkle_root'),
            'op_count': len(src_proofs),
        })

    merge_metadata = {
        'merged_at_utc': datetime.now(timezone.utc).isoformat(timespec='seconds'),
        'merged_at_unix': int(datetime.now(timezone.utc).timestamp()),
        'total_ops': len(all_proofs),
